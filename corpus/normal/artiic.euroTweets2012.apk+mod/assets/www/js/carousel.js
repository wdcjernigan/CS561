...
if ( typeof wargs.args.showNav === "boolean") {
	...
	if (wargs.args.scrollInterval) {
	    scrollInterval = wargs.args.scrollInterval;
	}
	if (wargs.args.itemHeight) {
            itemHeight = wargs.args.itemHeight;
        }
    }
    if (/%/i.test(baseWidth)) {
    ...
    this.addItem = function(item) {
        ...
        console.log("IF FILE/// -> " + text.indexOf("file:///android_asset/www/"));
        if (text.indexOf("file:///android_asset/www/") != -1) {
        	text1 = text.substring(0, text.indexOf("file:///android_asset/www/"));
            text2 = text.substring(text.indexOf("http://"));
            text = text1 + text2;
        }
        
        console.log("TEXT DESPUES -> " + text);
        ...
        _sel.bind(touchStopEvent,function(e) {
         ...
         console.log("addItem div montado");
    ...
    this.applyTemplate = function(obj, _t) {
        for (var i in obj) {
            ...
            if (i == "image") {
            	console.log("IMAGE!! -> " + obj[i]);
            	obj[i] = obj[i].substring(obj[i].indexOf("http://"));
            	console.log("IMAGE DESPUES!! -> " + obj[i]);
            }
            	
            
            while (_t.indexOf(token) != -1) {
            	console.log("_t antes: " + _t);
                _t = _t.replace(token, obj[i]);
                console.log("_t despues: " + _t);
            }
        }
        return _t;
    };
    ...
    this.setItems = function(_in) {
        ...
        for (var _i = 0; _i < data.length; _i++) {
        	console.log("setItems addItem data[_i] -> " + data[_i]);
    ...
    this.load = function() {
        ...
        if (wargs.value) {
            ...
            $("div[class='carousel-item']").each( function(index, item) {
            	
            	// Compruebo el tamaño porque si no da un error (se añade un elemento de mas inicialmente)
            	if (index < data.length) {
	            	if (data[index].activo == true) {
